...
|----------|---------------|---------------|
| AWS | ElastiCache for Redis | [IAM authentication](https://docs.aws.amazon.com/AmazonElastiCache/latest/red-ug/auth-iam.html) |
| GCP | Memorystore for Redis | [IAM authentication](https://docs.cloud.google.com/memorystore/docs/cluster/about-iam-auth) |
| Azure | Azure Cache for Redis | [Microsoft Entra authentication](https://learn.microsoft.com/en-us/azure/azure-cache-for-redis/cache-azure-active-directory-for-authentication) |
...
2. **Enable IAM authentication** on your Redis instance and grant access to your workload identity. Refer to your cloud provider's documentation linked above.
...
To enable IAM authentication, set the `iamAuthProvider` field and use an IAM-compatible connection string (with identity as username, no password):
...
For Redis Cluster with IAM authentication, configure both the cluster settings and IAM provider:
...
redis:
  external:
    ...
    iamAuthProvider: "gcp"  # or "aws" or "azure"
    cluster:
      enabled: true
      nodeUrisSecretKey: "redis_cluster_node_uris"
      tlsEnabled: true
...
You must apply the ServiceAccount annotations and pod labels required by your cloud provider's workload identity to all LangSmith components that connect to Redis. This includes:
