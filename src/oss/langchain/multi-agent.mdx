...
|----------|--------------|----------|
| [**Middleware handoffs**](#middleware-handoffs) | Agent calls a tool that updates state to transfer control | Dynamic routing, user-driven flows |
| [**Graph handoffs**](#graph-handoffs) | Graph edges define transitions between agents | Fixed pipelines, deterministic routing |

### Middleware handoffs

In **middleware handoffs**, agents pass control through **state transitions**. Think of it as a state machine where the system tracks which agent is currently active, and that agent's configuration (system prompt, tools, etc.) determines the behavior. For example:
...
<Card
    ...
    arrow cta="Learn more"
>
    Learn how to build a customer support workflow using middleware handoffs, where agents pass control through state transitions.
</Card>
