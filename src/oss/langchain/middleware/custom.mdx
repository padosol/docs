...
def add_context(
    ...
    # When content is a simple string
    ...
    # When content is a list of blocks, append a new text block
...
def add_cached_context(
    ...
    # Build new content with cache control
    # When content is a string, convert to list of blocks first
    if isinstance(request.system_message.content, str):
        base_content = [{"type": "text", "text": request.system_message.content}]
    else:
        base_content = list(request.system_message.content)

    # Add new content block with cache control
...
- `SystemMessage.content` can be either a `str` or a list of content blocks (e.g., `[{"type": "text", "text": "...", "cache_control": {...}}]`)
- When modifying system messages with structured content blocks, convert to a list first and append new blocks to preserve existing structure
